...
-- Basic RLS policies
CREATE POLICY IF NOT EXISTS "Agency admins can see their agency"
  ON public.agencies
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS "Agency admins can update their agency"
  ON public.agencies
  FOR UPDATE
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS "Users can see profiles in their agency"
  ON public.profiles
  FOR SELECT
  TO authenticated
  USING (
...
CREATE POLICY IF NOT EXISTS "Admins can update profiles in their agency"
  ON public.profiles
  FOR UPDATE
  TO authenticated
  USING (
